...
import { BookOpen, CalendarDays, ChartNoAxesColumnIncreasing, CircleUserRound, GraduationCap, Heart, House, List, PenLine, Repeat2, Settings, Sparkles, TextQuote } from 'lucide-react';
import type { ReactNode } from 'react';
import { supabase } from '@/lib/supabase';

const navigation = [
  { href: '/dashboard', label: 'Dashboard', icon: House }, { href: '/words', label: 'My Words', icon: BookOpen },
  { href: '/admin/words', label: 'Words List', icon: List }, { href: '/admin/quiz', label: 'Manage Quiz', icon: GraduationCap }, { href: '/admin/users', label: 'Users List', icon: CircleUserRound },
  ...
  { href: '/review', label: 'Review', icon: Repeat2 }, { href: '/quiz', label: 'Quiz', icon: GraduationCap }, { href: '/writing', label: 'Practice Writing', icon: PenLine }, { href: '/favorites', label: 'Favorites', icon: Heart },
...
export function AppShell({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const loadUser = async () => {
      ...
      setIsStaff(profile?.role === 'admin' || profile?.role === 'teacher');
    };
  ...
  return <div className="min-h-screen bg-[#f7f6f3] text-slate-900">
    ...
    <main className="pb-24 md:ml-64 md:pb-8"><header className="hidden h-16 items-center justify-end border-b border-stone-200 bg-white px-5 md:flex md:px-8"><div className="text-sm text-slate-500">Your personal Chinese class notebook</div></header><div className="mx-auto max-w-7xl p-4 sm:p-6 md:p-8">{children}</div></main>
    <nav className="fixed inset-x-0 bottom-0 z-10 flex items-center justify-around border-t border-stone-200 bg-white px-2 py-2 md:hidden">{[{href:'/dashboard',label:'Home',icon:House},{href:'/words',label:'Words',icon:BookOpen},{href:'/daily',label:'Daily',icon:CalendarDays},{href:'/quiz',label:'Quiz',icon:GraduationCap},{href:'/review',label:'Review',icon:Repeat2},{href:'/settings',label:'Profile',icon:CircleUserRound}].map(({href,label,icon:Icon}) => <Link key={label} href={href} className={`flex min-w-14 flex-col items-center gap-1 rounded-lg px-2 py-1 text-[11px] ${pathname === href ? 'text-[#b91c1c]' : 'text-slate-500'}`}><Icon size={19}/>{label}</Link>)}</nav>
  </div>
}
